...
void LOSGuidance::followPath(double x, double y, double psi)
{
  ...
  {
    lookahead_pose.pose.position.x = pose_a.pose.position.x + (proj_len+DELTA) * dx / seg_length;
    lookahead_pose.pose.position.y = pose_a.pose.position.y + (proj_len+DELTA) * dy / seg_length;

    double lookahead_dist = std::sqrt(std::pow(x - lookahead_pose.pose.position.x, 2) +
                                      std::pow(y - lookahead_pose.pose.position.y, 2));

    if (lookahead_dist <= 2*DELTA)
    {
  ...
  ROS_INFO("Going to (x: %.1f, y: %.1f) Lookahead point: (x: %.1f, y: %.1f)",pose_b.pose.position.x,pose_b.pose.position.y,lookahead_pose.pose.position.x, lookahead_pose.pose.position.y);
